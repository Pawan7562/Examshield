// src/components/QuestionForm.js - Comprehensive Question Form Component
...
const QuestionForm = ({ question, index, onChange, onRemove }) => {
  ...
  const renderMCQOptions = () => (
    <div>
      <div style={{ marginBottom: 16, fontSize: 14, fontWeight: 600, color: '#374151' }}>Options</div>
      {formData.options.map((option, index) => (
        <div key={index} style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
          {formData.options.map((option, index) => (
            <div key={index} style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 12, color: '#6b7280', minWidth: 30 }}>{String.fromCharCode(65 + index).toUpperCase()}.</span>
                <input
                  type="text"
                ...
                <input
                  type="radio"
                  name={`correct-${index}`}
                  checked={formData.correctAnswer === String.fromCharCode(65 + index).toLowerCase()}
                  onChange={() => handleOptionChange(index, 'isCorrect', String.fromCharCode(65 + index).toLowerCase())}
                  style={{ marginLeft: 8 }}
                />
